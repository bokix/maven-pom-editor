...
export class MavenUtils {
     ...
     * @returns Maven 命令路径
     */
    static getMavenCommand(): string {
        const config = vscode.workspace.getConfiguration('mavenPomEditor');
        const mavenPath = config.get<string>('mavenExecutablePath', 'mvn');
        return mavenPath.trim() || 'mvn';
    }
    ...
    static async getEffectivePom(pomFilePath: string): Promise<string> {
        try {
            ...
            // 构建 Maven 命令
            const command = `"${mavenCmd}" help:effective-pom -f "${pomFileName}" -Doutput=effective-pom.xml`;
    ...
    static async getDependencyTree(pomFilePath: string): Promise<string> {
        try {
            ...
            // 构建 Maven 命令
            const command = `"${mavenCmd}" dependency:tree -Dverbose -f "${pomFileName}"`;
            console.log(`执行 Maven 命令: ${command}`);
            console.log(`工作目录: ${workingDir}`);
    ...
    static async getResolvedDependencies(pomFilePath: string): Promise<string> {
        try {
            ...
            // 构建 Maven 命令
            const command = `"${mavenCmd}" dependency:list -f "${pomFileName}"`;
            console.log(`执行 Maven 命令: ${command}`);
            console.log(`工作目录: ${workingDir}`);
    ...
    static async isMavenAvailable(): Promise<boolean> {
        try {
            const mavenCmd = this.getMavenCommand();
            await execAsync(`"${mavenCmd}" --version`, { timeout: 5000 });
            return true;
        } catch (error: any) {
            return false;
        }
