(function() {
            ...
            <li class="tree-node ${highlightClass} ${selectedClass} ${omittedClass}" data-node-id="${nodeId}" data-level="${level}" data-artifact-id="${escapeHtml(node.artifactId)}" data-group-id="${escapeHtml(node.groupId)}">
                <div class="node-content">
                    ...
                    <span class="node-info">
                        ...
                        ${omittedLabel}
                        <button class="mvn-repo-btn" data-group-id="${escapeHtml(node.groupId)}" data-artifact-id="${escapeHtml(node.artifactId)}" title="View versions on mvnrepository.com">📦</button>
                    </span>
                </div>
        ...
        return `
            <li class="resolved-item ${highlightClass} ${selectedClass}" data-item-id="${itemId}" data-artifact-id="${escapeHtml(dep.artifactId)}" data-group-id="${escapeHtml(dep.groupId)}">
                <div class="resolved-item-content">
                    ...
                    <button class="mvn-repo-btn" data-group-id="${escapeHtml(dep.groupId)}" data-artifact-id="${escapeHtml(dep.artifactId)}" title="View versions on mvnrepository.com">📦</button>
                </div>
            </li>
        `;
    }
    ...
    function attachDependencyTreeListeners() {
        ...
        // 使用事件委托处理 mvn-repo-btn 点击
        const dependencyContainer = document.querySelector('.dependency-tree-container');
        if (dependencyContainer) {
            dependencyContainer.addEventListener('click', (e) => {
                if (e.target.classList.contains('mvn-repo-btn')) {
                    e.stopPropagation();
                    const groupId = e.target.getAttribute('data-group-id');
                    const artifactId = e.target.getAttribute('data-artifact-id');
                    vscode.postMessage({
                        type: 'openMvnRepository',
                        groupId: groupId,
        ...
        switch (message.type) {
            case 'update':
                if (monacoEditor) {
                    ...
                    // source code generated by GenAI of Kiro starts
                    // Use pushEditOperations instead of setValue to
                    // preserve the undo/redo stack
